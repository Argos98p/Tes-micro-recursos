...
public class RutaController {
    ...
    public ResponseEntity<?> addPlaceToRoute(@RequestBody Map<String,Object> body){
        ...
        Map<String,Object> ruta  = rutaService.agregarLugar((String) body.get("rutaId"), (ArrayList<String>) body.get("lugares"));
        if(ruta.isEmpty()){
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Ruta no encontrada");
        }
        return ResponseEntity.ok().body(ruta);
    ...
    public ResponseEntity<?> removePlaceToRoute(@RequestParam("rutaId") String rutaId, @RequestParam("lugarId")String lugarId){
        Map<String,Object> ruta  = rutaService.eliminarLugar(rutaId,lugarId);
        if(ruta.isEmpty()){
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Ruta no encontrada");
        }
        return ResponseEntity.ok().body(ruta);
